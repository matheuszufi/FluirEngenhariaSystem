...
const emptyForm = { name: '', company: '', role: '', text: '', stars: 5, imageUrl: '' }
...
function ImageUrlField({ value, onChange }) {
  ...
  return (
    <div className="faqadmin__field">
      ...
      />
      <span className="ta__url-hint">
        Google Drive: Compartilhar → <strong>Qualquer pessoa com o link</strong> → copie e cole aqui
      </span>
      {value && !error && (
        <div className="ta__preview" style={{ marginTop: 10 }}>
          <img src={value} alt="preview" onError={() => setError(true)} onLoad={() => setError(false)} />
        </div>
      )}
      {value && error && (
        <p className="ta__url-error">Imagem não carregou. Verifique se o arquivo está público no Google Drive.</p>
      )}
    </div>
  )
}
            ...
            <div key={item.id} className={`faqadmin__card${editingId === item.id ? ' faqadmin__card--editing' : ''}`}>
              {editingId === item.id ? (
                ...
                <div className="faqadmin__card-top">
                  {item.imageUrl
                    ? <img src={item.imageUrl} alt={item.name} className="ta__avatar ta__avatar--photo" />
