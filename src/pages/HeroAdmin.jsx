...
const emptyForm = { imageUrl: '', duration: 5 }
...
function SlideForm({ form, setForm, onSave, onCancel, label, saving }) {
    ...
    <div className="faqadmin__form">
      <div className="ha__form-grid">
        ...
        <div className="faqadmin__field">
          ...
          />
          <span className="pa__url-hint">Tempo visível antes de trocar para a próxima imagem</span>
        </div>
      </div>
...
export default function HeroAdmin() {
  ...
  const moveUp = async (index) => {
    ...
    await update(ref(rtdb, `hero/${a.id}`), { order: b.order ?? index - 1 })
    await update(ref(rtdb, `hero/${b.id}`), { order: a.order ?? index })
  }
  ...
  const saveAdd = async () => {
    ...
    setSaving(true)
    await push(ref(rtdb, 'hero'), {
      imageUrl: addForm.imageUrl.trim(),
      duration: Number(addForm.duration) || 5,
      order: slides.length,
    })
    cancelAdd()
    setSaving(false)
  ...
  const startEdit = (item) => {
    ...
    setEditForm({ imageUrl: item.imageUrl, duration: item.duration || 5 })
    setAdding(false)
  }
  ...
  const saveEdit = async (id) => {
    ...
    setSaving(true)
    await update(ref(rtdb, `hero/${id}`), {
      imageUrl: editForm.imageUrl.trim(),
      duration: Number(editForm.duration) || 5,
    })
    cancelEdit()
    setSaving(false)
  }
  ...
  const deleteItem = async (item) => {
    if (!window.confirm('Remover esta imagem do hero?')) return
    await remove(ref(rtdb, `hero/${item.id}`))
  }
  ...
  return (
    <DashboardLayout>
      <header className="dash__header faqadmin__header">
        <div>
          <h1 className="dash__header-title">Imagens do Hero</h1>
          <p className="dash__header-sub">
            Gerencie o slideshow de fundo da página principal.
            {!loading && <> <strong>{slides.length} imagem(ns)</strong> — sem imagens usa a padrão local</>}
          </p>
        </div>
        {!adding && (
        ...
        <div className="faqadmin__list">
          ...
          {adding && (
            <div className="faqadmin__card faqadmin__card--new">
              ...
              <SlideForm form={addForm} setForm={setAddForm} onSave={saveAdd} onCancel={cancelAdd} label="Salvar imagem" saving={saving} />
            </div>
          )}
          ...
          {slides.length === 0 && !adding && (
            <div className="faqadmin__empty">
              ...
              <p>Nenhuma imagem cadastrada. O hero usará a imagem padrão local <code>hero-bg.png</code>.</p>
            </div>
          )}
          ...
          {slides.map((item, index) => (
            editingId === item.id ? (
              <div key={item.id} className="faqadmin__card faqadmin__card--editing">
                <SlideForm form={editForm} setForm={setEditForm} onSave={() => saveEdit(item.id)} onCancel={cancelEdit} label="Salvar alterações" saving={saving} />
              </div>
            ) : (
              <div key={item.id} className="ha__card">
                <div className="ha__card-thumb">
                  <img src={item.imageUrl} alt={`Slide ${index + 1}`} loading="lazy"
                    onError={(e) => { e.target.style.display = 'none' }} />
                  <span className="ha__card-num">{index + 1}</span>
                </div>
                <div className="ha__card-info">
                  <p className="ha__card-url">{item.imageUrl}</p>
                  <span className="ha__card-duration">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
                      <circle cx="12" cy="12" r="10"/>
                      <polyline points="12 6 12 12 16 14"/>
                    </svg>
                    {item.duration || 5} segundos por imagem
                  </span>
                </div>
                <div className="ha__card-actions">
                  <div className="ha__reorder">
                    ...
                    <button onClick={() => moveDown(index)} disabled={index === slides.length - 1} title="Mover para baixo">↓</button>
                  </div>
